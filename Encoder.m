...
window_size=str2double(input('Enter block size (1 to 9):','s'));
while isnan(window_size) || window_size>9||window_size==0||fix(window_size) ~= window_size  
  window_size = str2double(input('Please enter INTEGER from 1 to 9: ', 's'));
end
...
case 3
        ...
        for i=1:4
        for j=1:4
        for k=1:4              %%P(X,Y,Z)=P(X)P(Y|X)P(Z|Y)
...
case 4
        ...
        for j=1:4
        for k=1:4
        for l=1:4     %%P(X,Y,Z,A)=P(X)P(Y|X)P(Z|Y)P(A|Z)
...
case 5
        ...
        for k=1:4
        for l=1:4
        for m=1:4   %%P(X,Y,Z,A,B)=P(X)P(Y|X)P(Z|Y)P(A|Z)P(B|A)
...
case 6
        ...
        for l=1:4
        for m=1:4
        for n=1:4  %%P(X,Y,Z,A,B)=P(X)P(Y|X)P(Z|Y)P(A|Z)P(B|A)
...
case 7
        ...
        my_data(i)=str2double(strcat(data(v),data(v+1),data(v+2),data(v+3),data(v+4),data(v+5),data(v+6)));
             v=v+7 ;
        end
        for i=1:4
        for j=1:4
        ...
        for m=1:4
        for n=1:4 
        for x=1:4   %%P(X,Y,Z,A,B)=P(X)P(Y|X)P(Z|Y)P(A|Z)P(B|A)
prob(t)=(indiv_prob(i)*trans_prob(i,j)*trans_prob(j,k)*trans_prob(k,l)*trans_prob(l,m)*trans_prob(m,n)*trans_prob(n,x));
symbols(t)=str2double(strcat(original_symbols_str(i),original_symbols_str(j),original_symbols_str(k),original_symbols_str(l),original_symbols_str(m),original_symbols_str(n),original_symbols_str(x)));         
        t=t+1;
        end
        end
        end
...
case 8
        ...
        my_data(i)=str2double(strcat(data(v),data(v+1),data(v+2),data(v+3),data(v+4),data(v+5),data(v+6),data(v+7)));
        v=v+8 ;
        end
        for i=1:4
        for j=1:4
        ...
        for n=1:4 
        for x=1:4  
        for y=1:4%%P(X,Y,Z,A,B)=P(X)P(Y|X)P(Z|Y)P(A|Z)P(B|A)
prob(t)=(indiv_prob(i)*trans_prob(i,j)*trans_prob(j,k)*trans_prob(k,l)*trans_prob(l,m)*trans_prob(m,n)*trans_prob(n,x)*trans_prob(x,y));
symbols(t)=str2double(strcat(original_symbols_str(i),original_symbols_str(j),original_symbols_str(k),original_symbols_str(l),original_symbols_str(m),original_symbols_str(n),original_symbols_str(x),original_symbols_str(y)));         
        t=t+1;                 
        end
        end
        end
    ...
    case 9
        ...
        for x=1:4  
        for y=1:4
        for z=1:4%%P(X,Y,Z,A,B)=P(X)P(Y|X)P(Z|Y)P(A|Z)P(B|A)
prob(t)=(indiv_prob(i)*trans_prob(i,j)*trans_prob(j,k)*trans_prob(k,l)*trans_prob(l,m)*trans_prob(m,n)*trans_prob(n,x)*trans_prob(x,y)*trans_prob(y,z));
symbols(t)=str2double(strcat(original_symbols_str(i),original_symbols_str(j),original_symbols_str(k),original_symbols_str(l),original_symbols_str(m),original_symbols_str(n),original_symbols_str(x),original_symbols_str(y),original_symbols_str(z)));         
        t=t+1;                 
        end
        end
        end
